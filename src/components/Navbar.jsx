  ...
  return (
    ...
    >
      <div className="container" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 1.25rem" }}>
        <motion.div whileHover={{ scale: 1.03 }} onClick={() => handleNav("#home")}
          style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          <span className="font-display" style={{ fontSize: "1.2rem", fontWeight: 900, color: "var(--ink)" }}>AP</span>
          <span style={{ color: "var(--yellow-dark)", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem" }}>◆</span>
        </motion.div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            {navItems.map((item) => (
              <button key={item.href} onClick={() => handleNav(item.href)}
                style={{ display: "block", width: "100%", background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: "0.9rem 1.25rem", fontFamily: "'Space Mono', monospace", fontSize: "0.8rem", letterSpacing: "0.1em", textTransform: "uppercase", color: active === item.href.replace("#", "") ? "var(--pista-deep)" : "var(--ink-muted)", borderBottom: "1px solid var(--pista-light)" }}>
                {item.label}
              </button>
            ))}
          </motion.div>
